...
Segments mostly contains from `type` and `template` fields. Where type is a value from enumeration presented next and template is handlebars template string. Data structure passed to template stored in variable named `v`. Somewhere it is single value, in other segments it may be structure.
